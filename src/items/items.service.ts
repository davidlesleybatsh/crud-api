...
import { search, searchAll, searchById } from "../js/elasticSearch";
...
export const findAll = async (): Promise<Item[]> => {
    ...
    const itemList = await searchAll();
    
    return itemList;
}

export const searchWithString = async (searchInput: any): Promise<Item[]> => {
    // TODO: space/split/trim searchInput;
    
    const itemList = await search(searchInput);
    
    return itemList;
}

export const findById = async (id: number): Promise<any> => { return await searchById(id)}
